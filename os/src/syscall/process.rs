...
use crate::{
    ...
    mm::{get_mut_data, translated_ref, translated_refmut, translated_str},
    task::{
        self, add_task, current_mmap, current_munmap, current_task, current_user_token, exit_current_and_run_next, pid2task, suspend_current_and_run_next, SignalAction, SignalFlags, MAX_SIG
    }, timer::get_time_us,
};
...
pub fn sys_get_time(_ts: *mut TimeVal, _tz: usize) -> isize {
    ...
    let us = get_time_us();
    let ts = get_mut_data(token, _ts).unwrap();
    unsafe {
        *ts = TimeVal {
            sec: us / 1_000_000,
            usec: us % 1_000_000,
